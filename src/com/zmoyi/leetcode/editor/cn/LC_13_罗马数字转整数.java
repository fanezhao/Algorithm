...
// 
//
// Related Topics 哈希表 数学 字符串 👍 2571 👎 0
...
public class LC_13_罗马数字转整数 {
    public static void main(String[] args) {
        Solution solution = new LC_13_罗马数字转整数().new Solution();
        ...
        System.out.println(solution.romanToInt2("IV"));
    ...
    class Solution {
        ...
        /**
         * 核心思路：罗马数字基本等于每个字符对应的数字之和，除了当前字符小于下一个对于的字符对应的数字
         * 从字符串的后端开始遍历每一个字符，并且将该字符对应的数据添加到总和。
         * 然后还要判断当前字符的前一个字符对于的数字是否小于当前字符。如果小于，则需要把该字符对应的数字从和里再减掉。
         *
         * @param s
         * @return
         */
        public int romanToInt(String s) {
            ...
            for (int i = s.length() - 1; i >= 0; i--) {
                ...
                sum += curr;
                if (i - 1 >= 0) {
                    int prevNum = getInt(s.charAt(i - 1));
                    if (prevNum < curr) {
                        sum -= prevNum;
                        i--;
                    }
        ...
        private int getInt(char symbol) {
            switch (symbol) {
                case 'I':
                    return 1;
                case 'V':
